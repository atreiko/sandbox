...
var fontsStyle = function fontsStyle() {
  // Файл стилей полключения шрифтов
  var fontsFile = "".concat(app.path.srcFolder, "/scss/fonts.scss"); // Проверяем существуют ли файлы шрифтоф 

  _fs["default"].readdir(app.path.build.fonts, function (err, fontsFiles) {
    if (fontsFiles) {
      //Проверяем уществует ли файл стилей для подключения шрифтов
      if (!_fs["default"].existsSync(fontsFile)) {
        ...
        // Если файл есть выводим соощение
