...
while (score) {
  ...
  // А теперь представьте, что таких проблем больше - сколько получиться вложенных if/else и насколько этот код будет читаемым для другого программиста?
  const scoreArr = score.split(': ');
  if (scoreArr[1]) {
    // если как минимум 2 элемента в массиве есть
    if (scoreArr[2]) {
      // если в массиве 3 и больше элементов
      alert("В названии предмета возможна только одна инструкция формата ': '");
    } else {
      const [discipline, score] = scoreArr;
      scores[discipline] = score; // все хорошо - добавляем оценку в табель
    }
  } else {
  ...
  try {
    const scoreArr = score.split(': ');
    // если что-то пошло не так - создаем ошибку сами с помощью конструкции throw new Error, а в качестве аргумента указываем текст ошибки. После инструкции throw new Error код дальше не выполняется, в итоге он становиться намного понятнее и проще читаемым
    if (scoreArr.length < 2) {
      throw new Error(
        'Название предмета и оценка не разделены точкой с запятой!',
      );
    }
    if (scoreArr.length > 2) {
      throw new Error(
        "В названии предмета возможна только одна инструкция формата ': '",
      );
    }
    if (!Number.isInteger(scoreArr[1]) || scoreArr[1] <= 0) {
      throw new Error('Оценка должна быть больше 0 и целым числом');
    }
    scores[scoreArr[0]] = scoreArr[1]; // если добрались сюда - значит ошибок нет и можно добавлять запиь в табель
  } catch (error) {
