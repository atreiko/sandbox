...
let sizes = [] //* will store all sizes
...
const validateForm = () => {
    if(!productName.value.length) {
        ...
        return showAlert('short description must between 10 to 100 letters long')
    } else if(!des.value.length) {
        return showAlert('enter detail description about the product')
    } else if(!imagePaths.length) { //* image link array
        return showAlert('upload at least one product')
...
if(location.pathname != '/add-product') {
    ...
    //? fetch the data if product not in session
